...
Data loading and KPI computation utilities for the Drug Safety Dashboard.
All heavy loads wrapped in @st.cache_data for performance.
Dataset loads automatically from Hugging Face — no CSV upload needed.
"""
import pandas as pd
import streamlit as st
import os
...
_CANDIDATE_PATHS = [
    _PROJECT_DIR / "data" / _CSV_NAME,
    _PROJECT_DIR / _CSV_NAME,
    _PROJECT_DIR.parent / _CSV_NAME,
    Path(os.getcwd()) / "data" / _CSV_NAME,
    Path(os.getcwd()) / _CSV_NAME,
]
...
@st.cache_data(show_spinner="Loading FDA FAERS dataset from Hugging Face...")
def load_from_huggingface() -> pd.DataFrame:
    """Load dataset directly from Hugging Face — no upload needed."""
...
def load_data(sample_n: int | None = None) -> pd.DataFrame:
    """
    Load FDA FAERS dataset.
    First tries local disk, then falls back to Hugging Face automatically.
    """
    # Try local first (faster if available)
    path = get_resolved_data_path()
...
def init_app_state():
    ...
    if "app_state" not in st.session_state:
        ...
        if model_exists:
            ...
            # Dataset always available via Hugging Face — skip APP_LOCKED
            st.session_state["app_state"] = "DATASET_UPLOADED"
